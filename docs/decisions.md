...
- Configurable transition timings live in CONFIG (MAP_FLY_FRAME_MS, SAILING_TRANSITION_MS, ANSWER_FEEDBACK_MS). Map fly-away uses a JS-driven sprite sequence (`MAP_FLY_FRAMES`) rather than CSS transform animation. Sailing and answer-feedback durations are pushed into CSS custom properties (--sail-ms, --answer-feedback-ms) at startup via applyAnimationTimings. Cosmetic-only durations (fade, map reveal, island entrance) stay as CSS variables in :root. SAILING_TIME_MS was renamed to SAILING_TRANSITION_MS; WIND_TRANSITION_MS was removed when the fly frames replaced the CSS blow-away.
...
- The sailing transition uses right-to-left ship movement (RTL feel: ship enters from the right, destination island on the left). Island size and ship dock position are configured separately in CONFIG (`SAILING_DESTINATION_ISLAND_SIZE_PERCENT`, `SAILING_SHIP_DOCK_LEFT_PERCENT`) as percents of the sailing scene. Positions use % of `#game-viewport`, not `vw`. Vertical bobbing is approximated with stepped CSS keyframes. Full-screen sailing background: `assets/ui/sailing_background.png` (CONFIG.SAILING_BACKGROUND_IMAGE), also used on question screens. Foreground sprites: `assets/ui/sailing_ship.png` and `assets/ui/destination_island.png`, with emoji fallbacks. Sailing text is plain over the sky (text-shadow), not a panel.
