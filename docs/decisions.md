...
- Image assets may be SVG (preferred for lightweight placeholders) or raster (PNG); both load as plain static files. Phase 7A added temporary SVG placeholders for the `gold` and `parrot` riddles only, to prove the pipeline. CONFIG.USE_IMAGE_ASSETS stays false by default so the prototype opens in placeholder mode (only 2 riddles have assets); set it to true to preview image/mixed mode.
