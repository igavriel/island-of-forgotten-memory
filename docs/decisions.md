...
- The visual map renders one randomized asset from each configured category. Asset pools live in `config/assets.js`; map positions and display sizes live in `CONFIG.MAP_ASSET_LAYOUT` as relative `x`, `y`, and `sizePercent` values. The map image is rendered as an in-screen `<img>` so positioned assets share its coordinate space.
